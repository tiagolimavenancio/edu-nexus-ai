...
import type { IQuestion, ISubmission } from "@/types/Exam";

interface IExamRadioProps {
  question: IQuestion;
...
const ExamRadio = ({ question: q, setAnswers, submission, answers }: IExamRadioProps) => {
  // 1. Get the student's answer for this specific question (if submitted)
  const studentAnswer = submission?.answers.find((a) => a.questionId === q._id)?.answer;
  ...
  return (
    ...
    >
      {q.options.map((opt, i) => {
        ...
        let containerStyle = "flex items-center space-x-2 p-3 rounded-md border transition-all ";
        let badgeText = "";
        let badgeColor = "";
        ...
        if (submission) {
          if (isThisOptionCorrect) {
            ...
            containerStyle += "bg-green-50 border-green-500 text-green-700 dark:bg-green-900/20";
            ...
            containerStyle += "bg-red-50 border-red-500 text-red-700 dark:bg-red-900/20";
        ...
        return (
          <div key={i} className={containerStyle}>
            <RadioGroupItem
              value={opt}
              id={`${q._id}-${i}`}
              className={submission && isThisOptionCorrect ? "text-green-600 border-green-600" : ""}
            />
            ...
            >
              ...
              {submission && badgeText && (
                <div className={`flex items-center gap-1 text-xs ${badgeColor}`}>
                  {isThisOptionCorrect ? (

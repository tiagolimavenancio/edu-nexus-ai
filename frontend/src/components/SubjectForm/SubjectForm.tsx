...
import { subjectFormSchema, type SubjectFormValues } from "@/schema/subjectForm";
...
export function SubjectForm({ open, onOpenChange, initialData, onSuccess }: ISubjectFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const teacherIds = initialData.teacher
        ? initialData.teacher.map((t: any) => (typeof t === "object" ? t._id : t))
        : [];
  ...
  const onSubmit = async (values: SubjectFormValues) => {
    ...
    try {
      ...
      const payload = {
        ...values,
        teacher: !values.teacher || values.teacher.length === 0 ? null : values.teacher,
      };
  ...
  return (
    <Modal
      ...
      description={
        initialData ? "Edit the subject details." : "Fill in the details to create a new subject."
      }
      open={open}
            ...
            render={({ field: { value, onChange, ...field }, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <div className="flex flex-row space-x-3 rounded-md border p-3">
                  <Checkbox id="isActive" checked={value} onCheckedChange={onChange} {...field} />
                  <div className="space-y-1">
                    <FieldLabel htmlFor="isActive" className="cursor-pointer mb-0">
                      Active Subject
                    </FieldLabel>
